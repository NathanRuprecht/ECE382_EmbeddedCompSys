...
#define	UP		0x02FD4887
#define DOWN	0x02FD40BF
#define	LEFT	0x02FDC03F
#define	RIGHT	0x02FD807F
...
void main(void) {
	...
	while(1)  {
		...
		if (irPacket == SKIP_RT) P1OUT |= BIT6; //turns on green led
		if (irPacket == SRCH_LT) P1OUT &= BIT6; //turns off green led
		if (irPacket == SRCH_RT) P1OUT |= BIT0; //turns on red led
		if (irPacket == SKIP_LT) P1OUT &= BIT0; //turns off red led
		newIrPacket = FALSE;
